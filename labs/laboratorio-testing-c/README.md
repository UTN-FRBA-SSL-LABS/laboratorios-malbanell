...
> R:carrito_init inicializa la cantidad de elementos en 0 (c->cantidad = 0). Es fundamental llamarla porque en C la memoria local del stack contiene basura; si no se inicializa, el carrito comenzaría con un conteo indeterminado e intentaría acceder a posiciones inválidas del array.
...
> R:Está dentro del bucle de carrito_total, en la línea que acumula: total += c->items[i].precio;

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R:Debería multiplicar el precio unitario por la cantidad de unidades de cada producto: total += c->items[i].precio * c->items[i].cantidad;
...
> R:Falla. El 5to intento devuelve 1 en lugar de 0.
...
> R:El operador incorrecto es > (o una comparación estricta) y debería ser >= MAX_ITEMS.
...
> R:No hay líneas con #####. Todos los flujos y ramas lógicas (casos exitosos, casos límite de capacidad y cálculos) están cubiertos por la suite de tests unitarios y de integración.
...
> R:Un test unitario evalúa una función o componente aislado de forma atómica e independiente. Un test de integración verifica cómo interactúan varias funciones entre sí (por ejemplo, agregar ítems, acumular subtotales y aplicar un descuento). El bug de carrito_total lo detectó primero el test unitario test_total_con_cantidad.
...
> R:C no realiza bounds checking automático en arreglos ni gestiona runtime checks de memoria; escribir fuera del límite sobrescribe memoria contigua (variables adyacentes, punteros del stack), provocando corrupción de datos, segfaults o vulnerabilidades de seguridad. Lenguajes con runtime gestionado como Python o Java verifican los índices e interrumpen la ejecución lanzando excepciones de forma segura (IndexError o ArrayIndexOutOfBoundsException).
...
> R:Definir tests formaliza el comportamiento esperado con aserciones objetivas y reproducibles. Evita el sesgo cognitivo de releer el código asumiendo que funciona, permite automatizar la verificación continua y protege contra regresiones futuras cada vez que se refactoriza.
...
> R:Para cantidad = 1, el cálculo erróneo (precio) y el correcto (precio * 1) producen exactamente el mismo resultado numérico ($350). Al no variar la cantidad, el caso de prueba no ejercitó la operación de multiplicación y enmascaró el defecto.
